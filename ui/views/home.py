...
st.subheader(
    f"Greetings, {st.session_state.user.get('display_name', 'Done')}! Where would you like to travel today?"
)
...
with col2:
    ...
    with st.container(border=True):
        ...
        st.write("")  # Spacer
        if st.button("View Side Quests ➔", key="btn_tasks", use_container_width=True):
